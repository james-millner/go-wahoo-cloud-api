...
func TestGetWahooOAuthExchangeURLL(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			wahooClientId:    "client123",
			wahooRedirectUri: "https://example.com/callback",
			expectedResult:   "https://api.wahooligan.com/oauth/token?client_id=client123&client_secret=%shttps://example.com/callback&code=%s123&grant_type=authorization_code&redirect_uri=https://example.com/callback",
			expectedError:    false,
		},
		{
			name:             "Empty input",
			wahooClientId:    "",
			wahooRedirectUri: "",
			expectedResult:   "https://api.wahooligan.com/oauth/token?client_id=&client_secret=%s&code=%s123&grant_type=authorization_code&redirect_uri=",
			expectedError:    false,
		},
		{
			name:             "Invalid redirect URI",
			wahooClientId:    "client456",
			wahooRedirectUri: "invalid_uri",
			expectedResult:   "https://api.wahooligan.com/oauth/token?client_id=client456&client_secret=%sinvalid_uri&code=%s123&grant_type=authorization_code&redirect_uri=invalid_uri",
			expectedError:    false,
		},
		// Add more test cases as needed
	}
